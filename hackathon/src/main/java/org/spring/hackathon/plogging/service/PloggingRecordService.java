...
public class PloggingRecordService {
  ...
  //플로깅 시작
  public Long ploggingStartDo(PloggingLocationDto location, String token, Long memberKey) {
    ...
    return ploggingRecord.getRecordKey();

  }

  //좌표 위치 업데이트
  @Transactional
  public PloggingLocationEntity ploggingLocationUpdate(PloggingLocationDto location, Long recordKey) {
    ...
    List<PloggingLocationEntity> locationList = ploggingLocationRepository.findAllByPloggingRecordForeignKey(recordKey);
  ...
  public void ploggingEndDo(PloggingRecordDto recordDto, PloggingLocationDto location, Long recordKey, MultipartFile image) throws IOException {
    ...
    //운동을 하는 회원 레코드 get
    Optional<MemberEntity> memberCheck = memberRepository.findById(finalRecordGet.getRecordJoinMember().getMemberKey());
    ...
    if(!image.isEmpty()) {
      ...
      imageService.imageSave(image, identify, recordKey);
    }
